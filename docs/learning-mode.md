...
- Phase 7 起以 CodeCAI 自身 CLI 能力为主线推进；Phase 9 起每个 Phase 开始前加入轻量 Reference Pass，只参考公开资料、成熟工程惯例、项目内已有实现和本地 reference-only 开源仓库。
...
3. 不复制源码，只抽象设计原则和取舍。
4. Reference notes 只保留：`问题 -> 参考项目做法 -> CodeCAI 采用什么 -> CodeCAI 暂不采用什么`。
...
- 不复制闭源、未授权或来源不明实现。
- 本地 `references/codex/`、`references/claude-code/` 等外部源码目录只作为 reference-only，不进入 CodeCAI 源码或 git 追踪范围。
