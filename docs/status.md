...
- `--task` 保留为一次性调试路径。
- 默认 model profile 是 `fake/fake`，不需要 API key；真实 provider 不再内置硬编码 model，必须通过 `/model-add` 或 `.codecai/models.json` 配置后才能使用。
...
- 工具模型已从单文件 `CodeCAI/tools.py` 拆到 `CodeCAI/tooling/` 分类模块；`CodeCAI.tools` 保留为兼容门面。
...
- LLM Provider runtime 已支持 `provider/model` model ref、`.codecai/models.json` profile 配置、`.env` API key 写入、动态 model discovery 和 lazy adapter cache；当前 provider 包括 `google`、`openai`、`anthropic`、`ollama`、`deepseek`、`glm`、`openai-compatible`，其中 `google` 使用 `GeminiAdapter`，`openai` / `deepseek` / `glm` / `openai-compatible` 复用 `OpenAICompatibleAdapter`。
- `CodeCAI/demand.py` 已定义 guided / workflow 可复用的 `DemandBrief` 执行合同和 `<demand_brief>` 渲染格式；`ContextComposer.compose(..., demand_brief=...)` 已能在 session context 后、当前 task 前注入 `<original_user_task>` 和 `<demand_brief>`，并要求冲突时保留更限制性的原始约束；`CodeCAI/guided.py` 当前负责 `ClarifyResult -> DemandBrief` 转换、DemandBrief preview 和 pending guided review 的业务规则，RuntimeSession 持有 `pending_guided_review` 状态并负责确认阶段路由。
- 普通 task 路径已支持 event streaming 数据源；Runtime 执行中展示临时 live process，完成后默认折叠，完整过程可通过 `Ctrl+O` 或 `/process` 展开。
- Workflow Planner / Compiler V1 已拆到 `CodeCAI/workflow/planner.py`；当前 `WorkflowPlanningAgent` 输出 `WorkflowPlanDraft`，`LLMWorkflowPlanningAgent` 可通过真实 `LLMAdapter` 生成 planner draft，`python -m CodeCAI.workflow.planner --task "..." --adapter gemini|fake` 可单独测试 planner，不经过 WorkflowRunner；旧 `python -m CodeCAI.workflow_planner ...` 仍作为兼容入口可用；`compile_workflow(task)` 先调用 planner draft，再按 `selected_template` 返回内置 `inspect_handoff` `WorkflowPlan(spec, script)`。
- Workflow Clarify V1 已拆到 `CodeCAI/workflow/clarify.py`；`ClarifyPhaseRunner` 每次只处理一个 clarify question，默认最多 8 轮；`LLMClarifyAgent` 可通过 read-only `read_file` / `list_files` / `glob_files` / `search_files` / `web_search` / `web_fetch` / `web_extract` 检查 repo 和公开网络资料后返回 `ask_question` / `complete` / `blocked` JSON；`ask_question` 支持 2-4 个选项并在 TTY 下通过选择弹窗返回 option value，用户可选择 Stop Clarify 或 Esc/Ctrl+C 取消，取消会 blocked 且不会进入后续执行；fake adapter workflow 路径使用 deterministic clarify，避免非 TTY smoke 卡在交互提问。
...
- 代码树提供 MIT License、贡献指南、安全报告入口和 GitHub Actions 测试流程。
- `.env`、`.codecai/models.json` 和 `.codex/config.toml` 作为本地配置排除版本控制，只保留无密钥示例。
- 公开定位是 Alpha 阶段 CLI Coding Agent 产品原型，不把长期路线描述为已完成能力。
- 公开仓库使用已清理历史；包含无再分发许可证源码快照的旧历史只保留在私有归档仓库中。
...
- `2026-07-13` 开源发布验证：Windows 运行 `python -m unittest discover tests`，335 个测试通过；CLI version / help / dry-run smoke 通过。
- GitHub Actions 在 Ubuntu + Python 3.10、3.11、3.12、3.13 上全部通过；公开配置示例通过 JSON / TOML / YAML 解析。
- 公开文件树和清理后 Git 历史分别通过 Gitleaks 0 命中扫描；GitHub secret scanning、push protection 和私密漏洞报告已启用。
- `python -m py_compile CodeCAI\workflow\__init__.py CodeCAI\workflow\core.py CodeCAI\workflow\clarify.py CodeCAI\workflow\planner.py CodeCAI\workflow\commands.py CodeCAI\workflow\runner.py CodeCAI\workflow_planner.py CodeCAI\workflow_commands.py CodeCAI\workflow_clarify.py`：exit code `0`。
...
- `python -m CodeCAI.workflow_planner --task "Read README" --adapter fake`：exit code `0`，确认旧 planner 兼容入口仍可用。
- `python -m CodeCAI.workflow.planner --task "Read README" --adapter fake --json`：exit code `0`，确认 planner 可单独输出 `WorkflowPlanDraft` JSON。
- `cmd /c "(echo /workflow Read README&echo /exit)|python -m CodeCAI --adapter fake --max-steps 3"`：exit code `0`，确认原 `/workflow` runtime 入口未回归。
- `python -m py_compile CodeCAI\runtime_commands.py CodeCAI\tui.py tests\test_runtime_commands.py tests\test_tui_status_bar.py tests\test_tui_completer.py`：exit code `0`。
...
- `python -m py_compile CodeCAI\demand.py tests\test_demand.py`：exit code `0`。
...
- `python -m py_compile CodeCAI\context.py CodeCAI\demand.py tests\test_context.py tests\test_demand.py`：exit code `0`。
...
- `python -m py_compile CodeCAI\guided.py CodeCAI\__main__.py CodeCAI\context.py tests\test_guided.py tests\test_runtime_session.py tests\test_context.py`：exit code `0`。
- `python -m py_compile CodeCAI\user_prompt.py CodeCAI\tui.py CodeCAI\workflow\clarify.py CodeCAI\guided.py tests\test_user_prompt.py tests\test_workflow_clarify.py tests\test_guided.py`：exit code `0`。
...
- `cmd /c "(echo /mode guided&echo Read README&echo /exit)|python -m CodeCAI --adapter fake --max-steps 3"`：exit code `0`，确认 guided mode 在 fake adapter 下 clarify complete、预览 `Guided demand brief` 后进入普通 Agent Loop。
...
- `python -m py_compile CodeCAI\tui.py tests\test_tui_status_bar.py tests\test_tui_completer.py`：exit code `0`。
...
- `2026-09-13` 项目名称重构验证：项目从 `OpenCAI` 正式重命名为 `CodeCAI`（主包名 `CodeCAI/`、CLI 入口 `python -m CodeCAI`、批处理 `CodeCAI\codecai.cmd`、配置目录首选 `.codecai/` 并向下兼容 `.opencai/`）。
  - `python -m py_compile` 覆盖改动文件通过。
  - `python -m unittest discover tests`：exit code `0`，全量 335 个测试全部通过。
  - CLI 冒烟验证：`--version`（`CodeCAI 0.0.0-dev`）、`--help`、`--dry-run`、`/workflow` smoke 均正常。
  - Benchmark 联通性验证：`python -m benchmarks.runner` 调起 `CodeCAI` 正常。
