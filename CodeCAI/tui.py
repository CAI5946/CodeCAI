...
try:
    ...
    from CodeCAI.composer import ComposerState, build_suggestions
    from CodeCAI.events import Event
    from CodeCAI.output_format import format_output_title
    from CodeCAI.runtime_commands import RUNTIME_COMMANDS
...
INPUT_PROMPT_LABEL = ""
INPUT_PLACEHOLDER = "Ask CodeCAI, type / for commands, or ! for shell"
INPUT_BORDER_CHAR = "─"
...
SELECT_PROMPT_STYLE_RULES = {
    ...
    "codecai-selected": "bold ansibrightcyan bg:default noreverse",
    "description": "ansibrightblack bg:default",
    "codecai-selected-description": "bold ansibrightcyan bg:default noreverse",
...
def _install_windows_shift_enter_compat() -> None:
    if ConsoleInputReader is None or KeyPress is None:
        return
    if getattr(ConsoleInputReader, "_codecai_shift_enter_compat", False):
        return
    ...
    ConsoleInputReader._codecai_shift_enter_compat = True
...
def _composer_suggestion_text_for_buffer(buffer: Buffer) -> str:
    origin = getattr(buffer, "_codecai_completion_origin", None)
...
def _has_composer_suggestions_for_buffer(buffer: Buffer) -> bool:
    ...
    if getattr(buffer, "_codecai_completion_dismissed_for", None) == text:
        return False
...
def _accept_active_completion_for_buffer(buffer: Buffer) -> bool:
    origin = getattr(buffer, "_codecai_completion_origin", None)
    selected_index = getattr(buffer, "_codecai_completion_index", None)
...
def _dismiss_composer_suggestions_for_buffer(buffer: Buffer) -> bool:
    ...
    _clear_composer_selection_for_buffer(buffer)
    setattr(buffer, "_codecai_completion_dismissed_for", text)
    buffer.cancel_completion()
    return True


def _clear_composer_selection_for_buffer(buffer: Buffer) -> None:
    setattr(buffer, "_codecai_completion_origin", None)
    setattr(buffer, "_codecai_completion_index", None)
...
def _select_composer_suggestion_for_buffer(buffer: Buffer, delta: int) -> bool:
    ...
    current_index = getattr(buffer, "_codecai_completion_index", None)
    ...
    setattr(buffer, "_codecai_completion_origin", origin)
    setattr(buffer, "_codecai_completion_index", next_index)
    return True
...
def create_task_buffer(
    ...
    *,
    ...
    setattr(buffer, "_codecai_history_entries", entries)
    setattr(buffer, "_codecai_history_index", len(entries))
    return buffer


def _refresh_completions(buffer: Buffer) -> None:
    _clear_composer_selection_for_buffer(buffer)
    if getattr(buffer, "_codecai_completion_dismissed_for", None) != buffer.text:
        setattr(buffer, "_codecai_completion_dismissed_for", None)
    if getattr(buffer, "_codecai_completion_dismissed_for", None) == buffer.text:
        return
...
def _history_entries_for_buffer(buffer: Buffer) -> tuple[str, ...]:
    entries = getattr(buffer, "_codecai_history_entries", ())
...
def _history_backward_for_buffer(buffer: Buffer) -> bool:
    ...
    index = int(getattr(buffer, "_codecai_history_index", len(entries)))
    if index <= 0:
        return True
    index -= 1
    setattr(buffer, "_codecai_history_index", index)
    ...
    *,
    ...
    def render_select() -> AnyFormattedText:
        ...
        for index, item in enumerate(items):
            ...
            style = "class:codecai-selected" if is_selected else "class:disabled" if item.disabled else "class:item"
            label_text = label_texts[index]
            rows.append((style, label_text))
            rows.append(("", " " * max(1, description_column - len(label_text))))
            if item.description:
                rows.append(("class:codecai-selected-description" if is_selected else "class:description", item.description))
            rows.append(("", "\n"))
        return rows
...
def show_keymap_view() -> None:
    from CodeCAI.runtime_commands import render_keymap_text
...
def ask_user_prompt_text(
    *,
    ...
    input_row = VSplit(
        [
            Window(
                FormattedTextControl([("class:codecai-selected", "? ")]),
                width=2,
                dont_extend_width=True,
            ),
            Window(
