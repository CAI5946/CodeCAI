...
from CodeCAI.agent_loop import iter_agent_loop
...
from CodeCAI.guided import PendingGuidedReview, handle_pending_guided_review, start_guided_review
from CodeCAI.llm_config import load_model_profiles, resolve_llm_config_path
from CodeCAI.llm_adapter import LLMAdapter, LLMAdapterError
...
from CodeCAI.runtime_commands import handle_runtime_command
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="codecai",
        description="Interactive runtime for the CodeCAI Alpha CLI Coding Agent prototype.",
    )
    parser.add_argument(
        "--task",
        default="",
...
def main() -> int:
    ...
    if (args.allow_write or args.allow_command) and args.permission == PermissionProfile.APPROVE_SAFE.value:
        permission_profile = PermissionProfile.FULL_ACCESS
    if args.dry_run:
        print(format_output_title("CodeCAI runtime"))
    ...
    try:
        ...
        print(f"CodeCAI adapter error: {exc}")
        return 1
