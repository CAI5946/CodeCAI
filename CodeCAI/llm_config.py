...
from CodeCAI.model_registry import ModelProfile, ModelRegistryError, model_profile_id


DEFAULT_LLM_CONFIG_PATH = Path(".codecai") / "models.json"
LEGACY_LLM_CONFIG_PATH = Path(".opencai") / "models.json"
LLM_CONFIG_ENV = "CODECAI_MODELS_CONFIG"
LEGACY_LLM_CONFIG_ENV = "OPENCAI_MODELS_CONFIG"


def resolve_llm_config_path(project_root: Path) -> Path:
    configured_path = os.environ.get(LLM_CONFIG_ENV, "").strip() or os.environ.get(LEGACY_LLM_CONFIG_ENV, "").strip()
    if configured_path:
        return Path(configured_path).expanduser()
    default_path = project_root / DEFAULT_LLM_CONFIG_PATH
    if not default_path.exists() and (project_root / LEGACY_LLM_CONFIG_PATH).exists():
        return project_root / LEGACY_LLM_CONFIG_PATH
    return default_path
