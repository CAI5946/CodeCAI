...
These tools define CodeCAI's extension boundary without pretending an MCP or
...
def _not_configured(tool_name: str) -> Any:
    def _run(arguments: dict[str, Any], cwd: Path) -> ToolResult:
        return tool_result(tool_name, False, error=f"{tool_name} is not configured in this CodeCAI runtime")
