...
from CodeCAI.tooling.contracts import ToolResult, ToolSpec, tool_result
from CodeCAI.tooling.path_utils import display_path, resolve_child_path


DEFAULT_SKILL_ROOT = "skills"
PROJECT_SKILL_ROOT = ".codecai/skills"
LEGACY_PROJECT_SKILL_ROOT = ".opencai/skills"
...
def _find_project_root(cwd: Path) -> Path:
    ...
    for candidate in [resolved, *resolved.parents]:
        if (
            (candidate / PROJECT_SKILL_ROOT).is_dir()
            or (candidate / LEGACY_PROJECT_SKILL_ROOT).is_dir()
...
def _default_invoke_skill_roots(cwd: Path) -> list[Path]:
    ...
    roots = [
        ...
        project_root / LEGACY_PROJECT_SKILL_ROOT,
...
SKILL_TOOLS: dict[str, ToolSpec] = {
    "list_skills": ToolSpec(
        name="list_skills",
        description="List local CodeCAI skills under a workspace skill root.",
        input_schema={
            "type": "object",
            "properties": {
                "root": {"type": "string"},
    ...
    "read_skill": ToolSpec(
        name="read_skill",
        description="Read the SKILL.md entrypoint for a local CodeCAI skill.",
        input_schema={
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    "invoke_skill": ToolSpec(
        name="invoke_skill",
        description=(
            "Load a local CodeCAI skill and return model-visible skill instructions. "
