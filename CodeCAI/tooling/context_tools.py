...
from CodeCAI.tooling.path_utils import display_path
...
def context_status(arguments: dict[str, Any], cwd: Path) -> ToolResult:
    from CodeCAI.context import ContextProvider
...
def search_memory(arguments: dict[str, Any], cwd: Path) -> ToolResult:
    return tool_result(
        "search_memory",
        False,
        error="search_memory is not configured; persistent memory indexing is outside the current CodeCAI runtime",
    )
...
CONTEXT_TOOLS: dict[str, ToolSpec] = {
    ...
    "search_memory": ToolSpec(
        name="search_memory",
        description="Search persistent CodeCAI memory if a memory backend is configured.",
        input_schema={
            "type": "object",
