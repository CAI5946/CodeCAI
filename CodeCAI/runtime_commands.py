...
from CodeCAI.adapter_factory import profile_from_adapter_name
from CodeCAI.llm_config import save_env_value, save_model_profile
from CodeCAI.llm_adapter import LLMAdapter, LLMAdapterError
from CodeCAI.model_discovery import list_provider_models
from CodeCAI.model_setup import MODEL_ADD_PROVIDER_CHOICES, PROVIDER_DEFAULTS, build_default_model_profile
from CodeCAI.model_smoke import run_model_smoke
...
def handle_runtime_command(
    ...
    if command == "/process":
        ...
        if not last_task_events:
            ...
            return False
        from CodeCAI.tui import show_process_view
        ...
        if model_registry is not None:
            try:
                ...
                try:
                    ...
                    print(f"CodeCAI model registry error: {exc}")
                    return False
        else:
            try:
                selected_adapter = adapter_factory(selected_model, api_key)
...
def _model_config_path(session: Any) -> Path:
    return Path(getattr(session, "model_config_path", Path(".codecai") / "models.json"))
